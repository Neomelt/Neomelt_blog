...
- 多图消息改为横向媒体预览：首图铺满预览框并按比例裁切，中央显示媒体数量，点击后可在全屏查看器中按原比例浏览和切换同组图片。
- 首张媒体预览优先加载并同步解码，减少进入日常页时的黑色占位闪现。
- Telegram 同步检查继续覆盖页面配置的全部保留范围；默认 `limit: 0`，不限制为固定条数。保留范围内的新增、删除和编辑都会触发站点重建。
- 补充 README、插件文档和操作指南，记录媒体展示、`latest.json` 指纹、手动工作流和故障跳过行为。
