...
- 网站构建时抓取配置范围内的消息（默认是整个公开频道历史），包括文字、图片、视频封面、链接卡片、点赞表情和浏览数，排成时间线。
...
- 一条消息里的图片会合并成一个媒体预览框；首张图片作为预览图铺满横向框，使用裁切来保持版面比例，不会在两侧留下空白。
- 预览框中央显示媒体图标和文件总数。点击后打开全屏查看器，图片按原始比例显示；多图可以用左右按钮或键盘方向键切换，右上角按钮或 `Esc` 关闭。
- 视频保留 Telegram 封面并显示跳转链接；浏览器无法直接展示的文件、贴纸、语音和投票仍会提示去 Telegram 查看原帖。
- 构建时首张预览图优先加载，降低进入 `/log` 时先看到黑色占位的时间；查看器沿用构建时生成的本地媒体地址。
...
2. `bin/check.mjs` 读取这个指纹，再按同一个保留条数抓取频道并比较。
3. 两者不一致时，才触发重新构建；指纹覆盖页面保留窗口，所以窗口内的新增、删除和编辑都会同步。

`limit: 0` 表示不限制，默认检查整个公开频道历史；只有显式设置正数时，页面和检查才会
共同使用该数量。检查不到状态文件或 Telegram 暂时不可访问时会记录 warning 并跳过本轮，
避免用不完整数据覆盖线上页面。
...
- 如果 Telegram 改动了网页结构，解析可能失效，需要更新 `core.mjs`。抓取失败时，构建不会中断，当前构建中的日志集合会清空，避免继续展示可能已经失效的本地媒体；空集合会显示空状态。
