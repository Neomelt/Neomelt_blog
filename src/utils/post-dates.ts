...
async function readFrontmatterDates(
  filePath?: string,
): Promise<RawFrontmatterDates> {
...
export async function resolvePostDates(
  ...
  const pubDatePart =
    parseDatePart(pubDateRaw) ?? fallbackDatePart(post.data.pubDate);
  const pubDate =
    explicitPubDate ??
    (gitDates.first
      ? createDate(
          pubDatePart,
          gitDates.first.hour,
          gitDates.first.minute,
          gitDates.first.offset,
        )
  ...
  if (explicitUpdatedDate) {
    ...
    const updatedDatePart =
      parseDatePart(updatedDateRaw) ?? fallbackDatePart(updatedBaseDate);
    if (gitDates.last) {
...
export async function resolvePostDatesMap(
  posts: PostEntry[],
): Promise<Map<string, ResolvedPostDates>> {
  const pairs = await Promise.all(
    posts.map(
      async (post) =>
        [getPostDateKey(post), await resolvePostDates(post)] as const,
    ),
  );
